...
Look at and run the code in `pwm-rgb-led.cpp` or `pwm-rgb_led.py`. You should see the red component of the LED smoothly pulse on and off.
...
Check out `pwm-piezo.cpp` or `pwm-piezo.py` and run it on the Pico. The buzzer will play some alternating tones.
# Info
