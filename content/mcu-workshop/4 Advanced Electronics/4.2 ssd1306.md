...
Upload the `ssd1306_i2c.cpp` or `oled.py` file to the Pico and watch the display print text, draw lines and flash on and off.
# Info
...
# Exercises
> [!exercise]
> Experiment around with the `main` function. See if you can edit the text that gets displayed, or draw some shapes with the `DrawLine` function.
